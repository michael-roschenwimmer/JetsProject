...
public class Hangar {
	...
	public Jet listLongRangeJetInFleet() {
		Jet longestRangeJet = currentJets[0];
		for (int i = 0; i < currentJets.length; i++) {
			if (currentJets[i].getRangeInMiles() > longestRangeJet.getRangeInMiles()) {
				longestRangeJet = currentJets[i];
			}
	...
	public Jet listFastestJetInFleet() {
		Jet fastestJet = currentJets[0];
		for (int i = 0; i < currentJets.length; i++) {
			if (currentJets[i].getSpeedMPH() > fastestJet.getSpeedMPH()) {
				fastestJet = currentJets[i];
			}
